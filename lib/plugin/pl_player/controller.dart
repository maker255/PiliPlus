...
class PlPlayerController with BlockConfigMixin {
  ...
  /// 退出全屏前的回调，由播放器视图注册，用于在退出全屏时自动还原视频的
  /// 缩放/旋转/平移（“还原屏幕”）。在执行退出全屏的方向/系统栏切换之前调用，
  /// 以避免与退出动画产生明显闪烁。
  VoidCallback? onBeforeExitFullScreen;
  ...
  Future<void> triggerFullScreen({
    ...
    try {
      if (status) {
        ...
        // 退出全屏前先还原视频的缩放/旋转/平移，再执行退出逻辑
        onBeforeExitFullScreen?.call();
        if (PlatformUtils.isMobile) {
          if (!removeSafeArea) {
            showSystemBar();
          }
